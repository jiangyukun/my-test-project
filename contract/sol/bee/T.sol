...
contract BeeRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last time that BEEs distribution occurs.
        uint256 accBeePerShare; // Accumulated BEEs per share, times 1e18. See below.
    ...
    uint256 public BeePerSecond = 0.00292566 ether; // 45500 bee / (180 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "BeeRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "BeeRewardPool: existing pool?");
        }
    }
    ...
    function safeBeeTransfer(address _to, uint256 _amount) internal {
        uint256 _beeBal = bee.balanceOf(address(this));
        if (_beeBal > 0) {
            if (_amount > _beeBal) {
                bee.safeTransfer(_to, _beeBal);
            } else {
