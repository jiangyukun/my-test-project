...
function handlePrototype(moduleName: string, code: string) {
  ...
  recast.visit(ast, {
    ...
    visitFunctionDeclaration(path: any): any {
      if (path.value.id.name == moduleName) {
        bodyAst.push(methodDefinition('constructor', identifier('constructor'), functionExpression(null, path.value.params, path.value.body)))
      } else {
  ...
  if (bodyAst.length != 0) {
    astResult.program.body.push(classDeclaration(identifier(moduleName), classBody(bodyAst)))
    if (unHandlePropertyList.length != 0) {
      console.log(moduleName, unHandlePropertyList)
    }
    let convertCode = recast.print(astResult).code += `
...
reserveFile('E:\\WEB-Projects\\mxgraph\\javascript\\src\\js')
