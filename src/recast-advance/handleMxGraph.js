...
function reserveFile(dir) {
  ...
  list.forEach(function (fileName) {
    ...
    if (stat && stat.isDirectory()) {
      ...
      let beforeLine = code.split('\n').length
      srcTotalLine += beforeLine
      let afterLine = resultInfo.code.split('\n').length

      if (resultInfo.otherModules.length > 0) {
        resultInfo.otherModules.forEach(subModuleName => {
          let otherModulePath = (dir + '/' + subModuleName + '.js')
          if (handledModuleList.indexOf(otherModulePath) != -1) {
            // console.log(`${filePath} ${subModuleName} 重复的模块`)
          } else {
            handledModuleList.push(otherModulePath)
            let subModuleResult = handleModule(subModuleName, code, otherModulePath)
            afterLine += subModuleResult.code.split('\n').length
          }
        })
      }
      let percent = ((beforeLine - afterLine) / beforeLine * 100).toFixed(2)
      let bLine = beforeLine.toString().padStart(5)
